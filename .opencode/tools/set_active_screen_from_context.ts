/**
 * set_active_screen_from_context - resolve a persisted .uos screen, then show it.
 */
...
export default tool({
  description:
    "Activate a Unity UI screen by resolving a persisted UOS screen from .uos context using screen name/id. Use for conversational requests like 'show the shop screen' when the canonical screenId is not already known.",
  args: {
    projectDir: z
      .string()
      .optional()
    ...
    refreshHierarchy: z.boolean().optional().describe("Best-effort refresh from live Unity hierarchy before resolving the screen. Defaults to true."),
  ...
  async execute(args, ctx) {
    ...
    if (!resolution.ok) {
      return {
        title: "set_active_screen_from_context: unresolved",
    ...
    const activateArgs = { screenId: resolution.screen.screenId };
    if (args.dryRun === true) {
      return {
        title: `set_active_screen_from_context: dry run ${resolution.screen.screenId}`,
        output: `Resolved screen=${formatScreen(resolution.screen.screenId, resolution.screen.screenName)}. Dry run only; Unity was not modified.` +
          formatLiveHierarchyRefresh(refreshed.refresh),
        metadata: { ok: true, dryRun: true, matched: resolution.screen, activateArgs, contextRefresh: refreshed.refresh },
      };
    }

    const activated = (await call("set_active_screen", activateArgs)) as Record<string, unknown>;
    return {
      title: `set_active_screen_from_context: ${resolution.screen.screenId}`,
      output: `Activated screen ${formatScreen(resolution.screen.screenId, resolution.screen.screenName)}. Other registered screens were hidden.` +
        formatLiveHierarchyRefresh(refreshed.refresh),
      metadata: {
        ok: true,
...
function formatScreen(screenId: string, screenName?: string): string {
  return `${screenId}${screenName !== undefined ? `(${screenName})` : ""}`;
}
...
function formatScreenFailure(reason: string, screens: Array<{ screenId: string; screenName?: string }>): string {
  ...
  for (const screen of screens.slice(0, 10)) {
    lines.push(`  - screen=${formatScreen(screen.screenId, screen.screenName)}`);
  }
  return lines.join("\n");
}
